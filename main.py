...
class Park:
    ...
    def add_user(self):
        ...
        user2 = User(name,phone,seria,age)
    ...
    def delete_user(self):
        ...
        for item in self.users:
            if item['name'] == name:
                print("Topildi")
                print(f"Name: {item['name']}, Seria: {item['seria']}, Phone: {item['phone']}, Age: {item['age']}")
    ...
    def view_users(self):
        ...
        for item in self.users:
            print("User View")
            count += 1
            print(f"{count}, Name: {item.name}, Seria: {item.seria}, Phone: {item.phone}, Age: {item.age}")

    def get_user(self,seria):
        for item in self.users:
            if item.seria == seria and item.is_active:
                return item
        return print("Malumot Topilmadi")
